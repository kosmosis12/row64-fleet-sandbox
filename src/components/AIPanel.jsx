...
import { queryAI, getProviderList } from "../ai/provider";

export default function AIPanel({ vehicle, onClose, onBack, aiProvider, aiApiKey, aiModel }) {
  ...
  const fetchLiveAI = async () => {
    ...
    const result = await queryAI(aiProvider || "claude", prompt, aiApiKey, aiModel);
    setLiveResponse(result); setLoading(false);
  };
  ...
  return <div style={{position:"absolute",top:10,left:10,width:340,zIndex:1002,background:"#fff",borderRadius:8,boxShadow:"0 4px 20px rgba(0,0,0,0.15)",border:"1px solid #e0e0e8",overflow:"hidden",maxHeight:"calc(100% - 20px)",overflowY:"auto"}}>
    <div style={{background:"#0078D4",padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:1}}>
      ...
      <div style={{display:"flex",gap:4,alignItems:"center"}}>
        <span style={{fontSize:8,color:"rgba(255,255,255,0.7)",textTransform:"uppercase"}}>{aiProvider || "claude"}</span>
    ...
    {!useLive ? <>
      ...
      <div style={{padding:"4px 12px 8px"}}>
        <button onClick={fetchLiveAI} style={{width:"100%",padding:"8px",background:"#f0f2f8",color:"#0078D4",border:"1px solid #d0d5e0",borderRadius:5,fontSize:10,fontWeight:700,cursor:"pointer"}}>🤖 Query Live AI ({aiProvider || "claude"})</button>
      </div>
    </> : <div style={{margin:"10px 12px"}}>
      {loading ? <div style={{padding:20,textAlign:"center",color:"#888",fontSize:11}}>
        <div style={{fontSize:20,marginBottom:8}}>🧠</div>Querying {aiProvider || "claude"}...
      </div> : <div style={{padding:"12px 14px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:6}}>
        <div style={{fontSize:10,fontWeight:800,color:"#166534",letterSpacing:"0.08em",marginBottom:6}}>AI RESPONSE ({(aiProvider||"claude").toUpperCase()})</div>
    ...
    <div style={{padding:"4px 12px 14px",display:"flex",gap:8,position:"sticky",bottom:0,background:"#fff"}}>
      <button onClick={()=>{onBack();setUseLive(false);setLiveResponse(null);}} style={{flex:0.4,padding:"10px",background:"#f0f2f8",color:"#555",border:"1px solid #ddd",borderRadius:5,fontSize:11,fontWeight:600,cursor:"pointer"}}>Back</button>
