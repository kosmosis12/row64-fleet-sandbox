...
const labelStyle = { fontSize: 10, fontWeight: 700, color: "#9898b0", letterSpacing: "0.06em", display: "block", marginBottom: 4, textTransform: "uppercase" };
const inputStyle = { width: "100%", padding: "5px 8px", background: "#1e1e2e", border: "1px solid #3a3a52", borderRadius: 3, fontSize: 11, color: "#e8e8f0", boxSizing: "border-box" };
const sectionStyle = { padding: "10px 12px", borderBottom: "1px solid #2a2a3e" };
...
export default function ConfigPanel({ config, setConfig, columnValues }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", overflowY: "auto", background: "#252538", color: "#e8e8f0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
          {CHART_TYPES.map((ct) => (
            <button key={ct.id} onClick={() => update({ chartType: ct.id })} style={{
              ...
              border: "1px solid " + (config.chartType === ct.id ? "#0078D4" : "#3a3a52"),
              background: config.chartType === ct.id ? "#0078D4" : "#1e1e2e",
              color: config.chartType === ct.id ? "#fff" : "#cfcfe0",
            }}>{ct.label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={sectionStyle}>
        ...
        {config.filters.length === 0 && (
          <div style={{ fontSize: 10, color: "#6b6b85", fontStyle: "italic" }}>No filters</div>
        )}
        {config.filters.map((f, i) => {
          const cd = colDef(f.column);
          return (
            <div key={i} style={{ marginBottom: 8, padding: 8, background: "#1e1e2e", border: "1px solid #3a3a52", borderRadius: 3 }}>
              <div style={{ display: "flex", gap: 4, marginBottom: 6 }}>
                ...
                <button onClick={() => removeFilter(i)} style={{
                  background: "transparent", color: "#ef4444", border: "1px solid #3a3a52",
              ...
              {cd?.type === "number" ? (
                ...
                <div style={{ maxHeight: 90, overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
                  {(columnValues[f.column] || []).map((v) => {
                    ...
                    return (
                      <label key={v} style={{ fontSize: 10, color: "#cfcfe0", display: "flex", alignItems: "center", gap: 4, cursor: "pointer" }}>
